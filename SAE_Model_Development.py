...
RANDOM_STATE = 42
TEST_SIZE = 0.3
N_FOLDS_CV = 10  # LASSO交叉验证折数
N_FOLDS_GRID = 3  # 超参数调优折数
DATA_FILE = "data.xlsx"  # 请替换为实际数据文件路径
OUTPUT_DIR = "./"  # 输出目录
...
def generate_simulated_data(n_samples=107, n_features=540, save_path=None):
    """
    生成模拟数据用于代码演示。
    真实数据因隐私限制不能公开，此函数生成具有相似统计特征的人工数据。
    """
    np.random.seed(RANDOM_STATE)

    # 生成三类标签: 0=Control, 1=non-SAE, 2=SAE
    n_control = 35
    n_nonsae = 38
    n_sae = 34
    ...
    # 使部分特征具有组间差异（模拟真实生物学信号）
    # APACHE II (假设在第0列)
    X[y == 0, 0] += -1.0  # Control 较低
    X[y == 1, 0] += 0.5  # non-SAE 中等
    X[y == 2, 0] += 1.5  # SAE 较高
...
def load_data(filepath):
    """
    加载数据文件。如果文件不存在，则生成模拟数据。
    """
    if not os.path.exists(filepath):
        print(f"⚠️ 数据文件 '{filepath}' 未找到，正在生成模拟数据用于演示...")
...
def perform_lasso_feature_selection(X, y, feature_names, cv=10):
    """
    使用LASSO回归进行特征选择，生成交叉验证曲线和系数路径图。
    """
    print("\n" + "=" * 50)
    print("1. LASSO特征选择")
    print("=" * 50)
...
def evaluate_model(model, X_test, y_test, classes, selected_features=None):
    ...
    if hasattr(model, "predict_proba"):
        try:
            ...
            metrics['AUC'] = roc_auc_score(y_test_bin, y_proba,
...
def perform_shap_analysis(model, X_train, feature_names, output_prefix):
    """对XGBoost模型进行SHAP分析（图5）"""
    print("\n" + "=" * 50)
    print("5. SHAP特征重要性分析")
    print("=" * 50)

    # 确保 feature_names 是 Python list 类型
    ...
    # 图5B: SHAP summary plot（兼容性版本）
    try:
        X_sample = X_train[:min(100, len(X_train))]

        # 使用 TreeExplainer（对XGBoost更稳定）
...
def main():
    ...
    col_order = ['AUC', 'Accuracy', 'Precision', 'Recall', 'F1',
                 'Kappa', 'Specificity', 'PPV', 'NPV']
    ...
    print("=" * 50)
    print(results_df.round(3).sort_values('AUC', ascending=False))
